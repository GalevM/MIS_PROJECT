...
class HomePage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final userName = userDoc.valueOrNull?['fullName']?.toString().split(' ').first ?? 'Граѓанин';
                ...
                children: [
                      ...
                      children: [
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text('Здраво, $userName! 👋', style: GoogleFonts.nunito(
                                fontSize: 18, fontWeight: FontWeight.w800, color: Colors.white,
                              )),
                              const SizedBox(height: 4),
                              Text('Да ги решиме заедно проблемите во нашата општина!', style: GoogleFonts.nunito(
                  ...
                  stats.when(
                    data: (s) => Padding(
                      ...
                      child: Row(
                        children: [
                          _StatCard(label: 'Решени', value: s['resolved'] ?? 0, color: AppTheme.success, icon: Icons.check_circle_outlined),
                          const SizedBox(width: 10),
                          _StatCard(label: 'Во тек', value: s['in_progress'] ?? 0, color: AppTheme.secondary, icon: Icons.timelapse_outlined),
                          const SizedBox(width: 10),
                          _StatCard(label: 'Примени', value: s['received'] ?? 0, color: AppTheme.warning, icon: Icons.inbox_outlined),
                        ],
                      ),
                    ),
                  ...
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 16),
                    child: Text('Брз пристап', style: theme.textTheme.titleMedium?.copyWith(
    ...
    return GestureDetector(
      ...
      child: Container(
        decoration: BoxDecoration(
          color: Colors.white,
          borderRadius: BorderRadius.circular(14),
          boxShadow: [BoxShadow(color: AppTheme.primary.withOpacity(0.08), blurRadius: 8)],
        ),
        child: Column(
          ...
          children: [
            ...
            Text(label, textAlign: TextAlign.center, style: GoogleFonts.nunito(
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Text((report.category as String).categoryEmoji, style: const TextStyle(fontSize: 24)),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text((report.category as String).categoryLabel, style: GoogleFonts.nunito(
                    fontSize: 14, fontWeight: FontWeight.w800, color: AppTheme.textPrimary,
                  )),
                  if ((report.address as String).isNotEmpty)
                    Text(report.address, style: GoogleFonts.nunito(fontSize: 12, color: AppTheme.textMuted), maxLines: 1, overflow: TextOverflow.ellipsis),
                ],
              ),
            ),
            Container(
              ...
              child: Text((report.status as String).statusLabel, style: GoogleFonts.nunito(
